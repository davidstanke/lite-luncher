...
async def save_favorite_menu(menu_description: str, tool_context: ToolContext) -> str:
    """Saves a user's favorite menu combination to the memory store.

    Args:
        menu_description: Description of the favorite menu option to save (e.g., 'Favorite menu: Tacos, Chips & Salsa, Lemonade, Churros').
    """
    formatted_pref = f"Favorite menu: {menu_description}"
    content = Content(parts=[Part(text=formatted_pref)], role="user")
    try:
        entry = MemoryEntry(content=content)
        await tool_context.add_memory(memories=[entry])
    except NotImplementedError:
        ...
        event = Event(content=content, author="user")
        await tool_context.add_events_to_memory(events=[event])
    return f"Saved favorite menu: {menu_description}"
...
luncher_agent = Agent(
    ...
    instruction=(
        ...
        - If the user prompt asks to save a menu option as a favorite (e.g., "Save Option 1 as my favorite" or "Save the Taco menu as a favorite"), call the `save_favorite_menu` tool to persist it, then confirm the saved favorite menu to the user. Do NOT perform any scheduling, meeting coordination, or menu queries in this case.
        ...
        - If the user request is a scheduling request, follow this exact linear execution pipeline:
          ...
          STEP 4: Synthesize the schedule and 3 distinct tailored menu options (main, 1-2 sides, drinks, dessert) with pricing breakdowns into a single final response. If any retrieved favorite menus comply with all attendee dietary constraints, preferentially offer that favorite menu as one of the 3 choices (marked as a favorite).
        ...
        - At the end of the scheduling response:
          ...
          - Inform the user that they can save any menu option as a favorite for future sessions (e.g., "You can save any menu option as a favorite by asking 'Save Option 1 as a favorite'.").
        """
